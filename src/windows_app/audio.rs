...
#[derive(Clone, Copy)]
struct SessionControlIdentityRef<'a> {
    key: &'a AudioSessionKey,
    token: usize,
}
...
fn same_session_group(left: &AudioSessionKey, right: &AudioSessionKey) -> bool {
    left.pid == right.pid && left.process_name == right.process_name
}

fn same_session_ownership(left: &AudioSessionKey, right: &AudioSessionKey) -> bool {
...
fn cached_session_requires_unmute(
...
fn should_retain_cached_session(
    desired_mute: bool,
...
impl TargetMuteStateUpdate {
    fn new(identity: TargetMuteIdentity<'_>, muted: bool) -> Self {
        Self {
...
#[derive(Clone, Copy)]
struct AudioSessionIdentity<'a> {
    process_name: &'a str,
    pid: u32,
}

fn target_update_matches(update: &TargetMuteStateUpdate, identity: TargetMuteIdentity<'_>) -> bool {
...
impl<'a> ManagedTargetLookup<'a> {
    ...
    fn has_recovery_match(&self, process_name: &str, pid: u32) -> bool {
        self.targets
            .iter()
    ...
    fn for_each_pid_target(&self, pid: u32, mut apply: impl FnMut(TargetMuteIdentity<'a>)) {
        for target in self
            .targets
            .iter()
    ...
    fn for_each_process_target(&self, mut apply: impl FnMut(TargetMuteIdentity<'a>)) {
        for target in self
            .targets
            .iter()
...
fn select_runtime_managed_session(
    key: &AudioSessionKey,
...
fn single_session_process_name(session_keys: &[AudioSessionKey]) -> Option<&str> {
