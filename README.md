...
첫 실행에서 `Windows 로그인 시 실행` 여부를 선택할 수 있습니다. 새 설정의 기본값은 `Windows 로그인 시 실행` 꺼짐, `시작 시 트레이로 최소화` 켜짐, `종료 시 앱 음소거 해제` 켜짐입니다. 첫 실행에서는 언어 선택과 초기 확인을 위해 메인 창을 한 번 표시하고, 이후 실행부터 트레이 최소화 설정이 적용됩니다.
