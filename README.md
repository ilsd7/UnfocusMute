...
Rust 네이티브 앱으로 빌드해 별도 런타임 없이 바로 실행됩니다. 현재 Windows용 실행 파일은 약 516KB로 1MB 미만입니다.
...
등록할 이름이 헷갈리면 먼저 게임을 실행해 둔 뒤, 게임 화면을 벗어나 Windows에서 `Ctrl`+`Shift`+`Esc`를 눌러 작업 관리자를 여세요. 프로세스 목록을 `CPU` 순으로 정렬하면 방금 실행한 게임을 찾기 쉽습니다. 게임 항목을 마우스 오른쪽 버튼으로 누르고 `속성`을 열면 `game.exe`처럼 `.exe`로 끝나는 프로세스 이름을 확인할 수 있습니다.
