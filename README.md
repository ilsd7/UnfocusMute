...
등록할 이름이 헷갈리면 게임을 실행한 뒤 `Ctrl`+`Shift`+`Esc`를 눌러 작업 관리자를 여세요. 프로세스 목록을 `CPU` 순으로 정렬하면 방금 실행한 게임을 찾기 쉽습니다. 게임 항목을 마우스 오른쪽 버튼으로 누르고 `속성`을 열면 `game.exe`처럼 `.exe`로 끝나는 프로세스 이름을 확인할 수 있습니다.
