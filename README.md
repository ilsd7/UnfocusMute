...
- `src/config.rs`: 설정 저장/로드와 프로세스 이름 정규화
...
앱 하단의 `언어` 선택 상자에서 한국어, English, 日本語, 简体中文 중 하나를 선택하면 즉시 UI가 갱신되고 설정이 자동 저장됩니다.
...
UnfocusMute는 등록 프로세스 이름, UI 언어, 시작 옵션만 로컬 JSON 설정에 저장합니다. 오디오 세션 감지와 뮤트 제어는 Windows CoreAudio API로 로컬에서만 처리되며, 네트워크 요청, 분석 도구, 크래시 리포팅, 원격 로깅을 포함하지 않습니다.
