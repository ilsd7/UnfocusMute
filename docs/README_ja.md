...
- 標準の一覧で音声セッションがあるアプリを選び、必要に応じて `全プロセス` へ切り替えるか、`game.exe` のような名前を直接入力
...
標準の選択一覧には、現在音声セッションがあるアプリだけが表示されます。まだ音声セッションを作成していないアプリは、`全プロセス` に切り替えると実行中の `.exe` 一覧から探せます。
...
4. `プロセスを検索` でアプリを選ぶか、検索語を入力して見つけたうえで `登録` を押します。まだ音声セッションを作成していないアプリは `全プロセス` に切り替え、実行中のすべてのプロセス一覧から探せます。一覧にない場合は `.exe` 名を直接入力してください。
