<div align="center">
  ...
  <p><strong>選択したゲームやアプリが前面でなくなると自動でミュートする、軽量なWindowsトレイアプリです。</strong></p>
...
Rust製のネイティブアプリなので、別途ランタイムを用意せずにそのまま実行できます。実行ファイルのサイズは約500 KBです。
...
- 標準の一覧で音声セッションがあるアプリを選び、必要に応じて `すべてのプロセス` へ切り替えるか、`game.exe` のような名前を直接入力
