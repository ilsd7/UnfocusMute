...
- 音声セッション一覧から選択、必要に応じて実行中の全プロセス一覧へ切り替え、または `game.exe` のような名前を直接入力
...
UnfocusMuteは、Windowsが提供するプロセス名、前面ウィンドウ情報、CoreAudioセッションを基準に動作します。標準の選択一覧には音声セッションがあるアプリが表示されます。まだ音声セッションを作成していないアプリは `全プロセス` に切り替えると、実行中の `.exe` 一覧から選べます。ただし、ドライバー、権限設定、セキュリティソフトがセッションへのアクセスを制限している場合、ミュート制御は引き続き制限されることがあります。
...
4. `プロセスを検索` の一覧を開くか検索語を入力し、アプリを選んで `登録` を押します。まだ音声セッションを作成していないアプリは `全プロセス` に切り替えて探し、それでも表示されない場合は `.exe` 名を直接入力してください。
