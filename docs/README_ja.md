...
4. `Alt`+`Tab` で別のウィンドウに切り替えてから、元のアプリに戻ってみてください。登録したアプリがバックグラウンドにある間はミュートされ、前面に戻ると音が復元されます。
...
**タスクトレイでの動作と音の復元:** 登録したアプリがミュートされたまま終了すると、Windowsが最後のミュート状態を記憶することがあります。UnfocusMuteがタスクトレイで動作していれば、アプリを再び起動して前面に出したときに音を自動で復元します。UnfocusMuteも終了したあとにアプリから音が出ない場合は、Windowsの `ボリューム ミキサー` で手動でミュートを解除してください。

**PID登録:** Windowsが通知するオーディオセッションのPIDと前面ウィンドウのPIDは、異なる場合があります。そのためPIDで登録していても、同じ `.exe` 名のウィンドウが前面に来ると、そのアプリが戻ったものと判断して音を復元します。したがって、同じ `.exe` の別ウィンドウを使いながら特定のPIDだけをミュートし続ける用途には向きません。一方、別のアプリで作業しながら、同じ `.exe` の複数PIDのうち一つだけをミュートし、ほかのPIDの音は残したい場合には便利です。
...
- **音が復元されない:** アプリをもう一度前面に出してください。UnfocusMuteをすでに終了している場合は、Windowsの `ボリューム ミキサー` で手動でミュートを解除してください。
