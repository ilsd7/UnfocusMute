...
- 音声セッションがあるアプリ一覧から選択して登録、または `game.exe` のような名前を直接入力
...
4. `プロセスを検索` の一覧を開くか検索語を入力し、音声セッションがあるアプリを選んで `登録` を押します。まだ音声セッションを作成していないアプリは一覧に表示されない場合があるため、その場合は `.exe` 名を直接入力してください。
