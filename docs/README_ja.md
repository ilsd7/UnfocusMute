...
- 実行中のアプリ一覧から選択して追加、または `game.exe` のような名前を直接入力
...
PID登録では、1つ知っておくべき動作があります。WindowsはオーディオセッションのPIDと前面ウィンドウのPIDを常に同じ値で返すとは限りません。これを補正するため、UnfocusMuteは登録したPIDの `.exe` 名と現在前面にあるウィンドウの `.exe` 名が同じ場合、そのアプリが前面に戻ったものとして扱います。そのため、同じ `.exe` を複数起動している環境では特定PIDだけを完全には分離できず、別のインスタンスが前面にある場合でも音声が復元されることがあります。
...
セキュリティ上、ユーザーは開発者がリポジトリに公開されたコードとは異なるファイルを悪意を持って配布した場合や、アカウント乗っ取りなどによって配布ファイルが改ざんされた場合に備えられる必要があります。そのためには、GitHub Releasesにアップロードされたファイルが公開ソースコードと一致する公式ビルドかどうかを、ユーザー自身が直接検証できる手順が必要です。

セキュリティと透明性のため、UnfocusMuteはGitHub Releasesにアップロードされた配布ファイルが、このリポジトリのソースコードと一致する公式ビルドかどうかをユーザーが直接確認できる検証方法を提供します。

リリースZIPファイルとSHA-256チェックサムファイルは、GitHubの自動ビルドシステムであるGitHub Actionsによって作成され、どちらのファイルも出所を証明するattestationとともに提供されます。

以下のコマンドを使うと、GitHub ReleasesからダウンロードしたZIPファイルがこのリポジトリの公式ビルドと同一であることを検証できます。
