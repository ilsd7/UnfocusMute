...
4. انتقل إلى نافذة أخرى باستخدام <span dir="ltr"><kbd>Alt</kbd>+<kbd>Tab</kbd></span> ثم عُد. يُكتم التطبيق المسجل عندما يكون في الخلفية، ويُستعاد صوته عند عودته إلى المقدمة.
...
**التشغيل في علبة النظام واستعادة الصوت:** إذا أُغلق تطبيق مسجل وهو مكتوم، فقد يتذكر Windows آخر حالة كتم له. ما دام UnfocusMute قيد التشغيل في علبة النظام، فسيستعيد الصوت تلقائيًا عند فتح التطبيق من جديد ونقله إلى المقدمة. إذا أنهيت UnfocusMute أيضًا وظل التطبيق بلا صوت، فألغِ كتمه يدويًا من `خالط مستوى الصوت` في Windows.

**التسجيل باستخدام PID:** قد يعيّن Windows قيم PID مختلفة للجلسة الصوتية وللنافذة الموجودة في المقدمة. لذلك، حتى عند التسجيل باستخدام PID، يعتبر UnfocusMute أن التطبيق عاد عندما تنتقل نافذة تحمل اسم ملف <code dir="ltr">.exe</code> نفسه إلى المقدمة، ثم يستعيد صوته. لهذا لا يناسب هذا الأسلوب من يريد استخدام نافذة أخرى من ملف <code dir="ltr">.exe</code> نفسه مع إبقاء PID محدد مكتومًا. لكنه قد يكون مفيدًا إذا أردت، أثناء العمل في تطبيق آخر، كتم صوت PID واحد فقط من بين عدة قيم PID لملف <code dir="ltr">.exe</code> نفسه مع الإبقاء على أصوات البقية.
...
- **لا يُستعاد الصوت:** أعد التطبيق إلى المقدمة. إذا كنت قد أنهيت UnfocusMute، فألغِ كتم التطبيق يدويًا من `خالط مستوى الصوت` في Windows.
