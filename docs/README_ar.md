...
UnfocusMute تطبيق مستقل لا يحتاج إلى تثبيت. كما لا تحتاج إلى تثبيت بيئة تشغيل منفصلة أو أدوات تطوير مثل Rust أو Visual Studio Build Tools أو MinGW.
...
4. اختر تطبيقًا من القائمة أو أدخل اسم <code dir="ltr">.exe</code> الدقيق، ثم اضغط `تسجيل`. التسجيل باسم ملف <code dir="ltr">.exe</code> يدير جميع الجلسات الصوتية لذلك التطبيق معًا؛ استخدم التسجيل باستخدام <code dir="ltr">PID</code> إذا أردت إدارة نسخة تشغيل محددة فقط. إذا لم يكن التطبيق قد أنشأ جلسة صوتية بعد، فانتقل إلى `كل العمليات` لاستعراض جميع العمليات الجارية.
