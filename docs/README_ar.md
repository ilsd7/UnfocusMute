...
- اختر تطبيقًا من القائمة الافتراضية للتطبيقات التي لديها جلسة صوت؛ وعند الحاجة انتقل إلى `كل العمليات` أو اكتب اسمًا مثل `game.exe`.
...
4. اختر تطبيقًا من `ابحث عن عملية` أو اكتب كلمة بحث للعثور عليه، ثم اضغط `تسجيل`. إذا لم يكن التطبيق قد أنشأ جلسة صوت بعد، فانتقل إلى `كل العمليات` لتصفح قائمة كل العمليات الجارية. استخدم `جلسات الصوت فقط` للعودة إلى القائمة المصفاة. إذا لم يكن في القائمة، فاكتب اسم `.exe` يدويًا.
5. إذا احتجت إلى تسجيل PID محدد فقط، اضغط `عرض PID` واختر الإدخال المطلوب. التسجيل عبر PID ينطبق فقط على النسخة الحالية قيد التشغيل؛ إذا أُعيد تشغيل التطبيق وتغيّر PID، فاختره من جديد.
