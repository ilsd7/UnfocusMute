...
UnfocusMute هو تطبيق صغير وخفيف لعلبة نظام Windows. عندما تنتقل لعبة أو تطبيق قمت بتحديده إلى الخلفية، يكتم تلقائيًا جلسة الصوت الخاصة بذلك التطبيق. لا يقتصر على الألعاب؛ يمكن أيضًا تسجيل المتصفحات، وتطبيقات المحادثة، والمشغلات، ومشغلات الوسائط، وأي تطبيق يظهر كجلسة صوت في Windows.

لأنه مبني كتطبيق Rust أصلي، يعمل مباشرة دون runtime منفصل. يبلغ حجم الملف التنفيذي نحو 500 كيلوبايت.
...
- يسجل الأهداف من قائمة التطبيقات قيد التشغيل أو عبر كتابة اسم مثل `game.exe`.
- يدعم أهداف `.exe`، وأهداف PID للنسخة الحالية، وخيار `عرض PID`.
- ملاحظات لكل تطبيق، وحالة كتم مباشرة لكل هدف، و`إيقاف مؤقت` و`استئناف` لكل تطبيق.
- البقاء في علبة النظام، ملخص حالة في علبة النظام، إيقاف مؤقت عام، فتح مجلد الإعدادات، ومنع تشغيل نسخ متعددة.
- يجمع زر الإعدادات في الزاوية السفلية اليسرى خيارات السلوك واللغة ومجلد الإعدادات وGitHub ومعلومات الإصدار.
...
1. شغّل UnfocusMute.
2. اختر اللغة من نافذة الاختيار عند التشغيل الأول. اللغة الافتراضية هي الإنجليزية.
...
5. إذا احتجت إلى تسجيل PID محدد فقط، اضغط `عرض PID` واختر العنصر المنفصل. الأهداف المسجلة عبر PID تنطبق فقط على النسخة الحالية قيد التشغيل؛ إذا أُعيد تشغيل التطبيق وتغيّر PID، فاختره من جديد.
...
8. عند إغلاق النافذة، يبقى التطبيق في علبة النظام ويواصل المراقبة. استخدم `خروج` لإنهائه بالكامل.

## استخدام ملاحظات التطبيقات المسجلة

إذا كان اسم العملية وحده لا يكفي لتذكّر التطبيق، فانقر بزر الفأرة الأيمن على التطبيق المسجل واختر `تعديل الملاحظة`. تظهر الملاحظة فوق اسم العملية في قائمة التطبيقات المسجلة، ولا تؤثر في طريقة مطابقة الهدف.
...
كاستثناء، عندما تضغط زر GitHub في شاشة الإعدادات، تُفتح صفحة GitHub الخاصة بهذا المشروع في المتصفح الافتراضي.

**ما يحفظه:** أسماء العمليات المسجلة، وPID الاختيارية، والملاحظات التي تكتبها، واللغة المختارة، وموضع النافذة، وخيارات بدء التشغيل، وحالة استعادة الكتم للأهداف المسجلة.
تُحفظ هذه البيانات فقط في `%APPDATA%\UnfocusMute\config.json` ولا تُرسل إلى أي مكان خارجي.

**ما لا يحفظه:** لا ينشئ ملفات سجل للتطبيق. ولا يحتفظ بأي سجل لسلوك الجلسات بين مرات التشغيل.

**ما لا يفعله:** لا يرسل طلبات شبكة تلقائية، ولا يستخدم telemetry، ولا يرسل تقارير أعطال، ولا يسجل عن بُعد، ولا يجمع البيانات. ولا يحتاج إلى صلاحيات مسؤول.
...
يتم إنشاء ملف ZIP الخاص بالإصدار وملف checksum بصيغة SHA-256 تلقائيًا عبر GitHub Actions، ويأتي كل ملف مع attestation يمكنها إثبات مصدر البناء.

تتيح لك الأوامر أدناه التحقق من أن ملف ZIP الذي حمّلته تم إنشاؤه بواسطة البناء الرسمي لهذا المستودع.
