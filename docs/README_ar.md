...
- اختر من قائمة جلسات الصوت، أو انتقل عند الحاجة إلى قائمة كل العمليات الجارية، أو اكتب اسمًا مثل `game.exe`.
...
يعتمد UnfocusMute على أسماء العمليات ومعلومات النافذة الموجودة في المقدمة وجلسات CoreAudio التي يوفرها Windows. تعرض القائمة الافتراضية التطبيقات التي لها جلسة صوت؛ انتقل إلى `كل العمليات` لاختيار عمليات `.exe` الجارية التي لم تنشئ جلسة صوت بعد. وإذا حدّ برنامج تشغيل أو إعداد صلاحيات أو برنامج أمان من الوصول إلى الجلسة، فقد يبقى التحكم في الكتم محدودًا.
...
4. افتح قائمة `ابحث عن عملية` أو اكتب كلمة بحث، ثم اختر تطبيقًا واضغط `تسجيل`. استخدم `كل العمليات` إذا لم ينشئ التطبيق جلسة صوت بعد؛ وإذا لم يظهر أيضًا، فاكتب اسم `.exe` يدويًا.
