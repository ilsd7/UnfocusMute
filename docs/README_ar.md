...
4. اختر تطبيقًا من القائمة أو أدخل اسم <code dir="ltr">.exe</code> الدقيق، ثم اضغط `تسجيل`. التسجيل باسم ملف <code dir="ltr">.exe</code> يدير جميع الجلسات الصوتية لذلك التطبيق معًا. إذا لم يكن التطبيق قد أنشأ جلسة صوتية بعد، فانتقل إلى `كل العمليات` لاستعراض جميع العمليات الجارية.
