...
- 可从音频会话列表中选择，必要时切换到所有运行进程列表，也可直接输入 `game.exe` 这样的名称
...
UnfocusMute 依赖 Windows 提供的进程名、前台窗口信息和 CoreAudio 会话工作。默认选择列表显示已有音频会话的应用；如果应用尚未创建音频会话，可切换到 `所有进程`，从正在运行的 `.exe` 列表中选择。不过，如果驱动、权限设置或安全软件限制了会话访问，静音控制仍可能受限。
...
3. 启动你想在后台时静音的游戏或应用。
4. 打开 `搜索进程` 列表或输入搜索词，选择应用后点击 `注册`。如果应用尚未创建音频会话，请切换到 `所有进程` 查找；如果仍未显示，请手动输入 `.exe` 名称。
...
发布构建已针对较小体积进行配置。`Cargo.toml` 的 release 配置会剥离符号、启用 LTO、使用单个 codegen unit、设置 `panic = "abort"`，并优先优化体积。
