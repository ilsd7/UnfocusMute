<div align="center">
  ...
  <p><strong>轻量级 Windows 托盘应用，可在所选游戏或应用失去焦点时自动静音。</strong></p>
...
UnfocusMute 是一款小巧轻量的 Windows 托盘应用，可在所选游戏或应用转入后台时自动静音，并在回到前台时解除静音。

- 基于 Rust 原生构建，无需额外运行时即可直接运行。
- 可执行文件大小约 600 KB。
- 它并不局限于游戏，也可以添加浏览器、聊天工具、启动器、媒体播放器等普通应用。
- UnfocusMute 只会自动解除由自身设置的静音，用户手动静音的应用会保持原样。
...
4. 使用 `Alt`+`Tab` 切换到其他窗口，再切换回来。已添加的应用进入后台时会被静音，回到前台时会解除静音。

这样就设置好了。添加应用后会立即开始监控；默认情况下，即使关闭 UnfocusMute 窗口，它也会继续在系统托盘中运行。

> **列表里找不到应用？** 点击 `所有进程`，或直接输入准确的 `.exe` 名称。如果只想添加当前正在运行的某个 PID，请使用 `PID 视图`。应用每次重新启动时 PID 都会变化，因此大多数情况下按 `.exe` 名称添加更合适。
...
**托盘运行与解除静音：** 已添加的应用回到前台或你退出 UnfocusMute 时，UnfocusMute 都会自动解除静音。已添加的应用先退出时，UnfocusMute 也会清除残留的静音状态。不过，如果 UnfocusMute 异常退出，Windows 中仍可能保留静音状态。之后启动应用却听不到声音时，请在 Windows 的 `音量合成器` 中检查该应用的静音状态。

**按 PID 添加：** Windows 提供的音频会话 PID 和前台窗口 PID 可能不同。因此，即使按 PID 添加，只要同一 `.exe` 名称的窗口切换到前台，UnfocusMute 也会认为你已切回该应用并解除静音。因此，这种方式不适合在继续使用同一 `.exe` 的窗口时，让某个特定 PID 始终保持静音。反过来，如果你希望在使用其他应用时，只将同一 `.exe` 的多个 PID 中的某一个静音，同时保留其余 PID 的声音，这项功能会很有用。
...
- **无法解除静音：** 切回已添加的应用后仍听不到声音时，请在 Windows 的 `音量合成器` 中检查该应用的静音状态。
- **按 PID 添加的行为不符合预期：** 请参阅[按 PID 添加的行为](#需要了解的行为)。
