...
- 作为 Rust 原生应用构建，无需额外运行时即可直接运行。
- 可执行文件目前约为 500 KB。
- 静音和恢复只会作用于 UnfocusMute 自己改动过的会话；你原本已经静音的会话不会被改动。
...
- 已注册应用转入后台时自动静音，回到前台时恢复声音
- 直接选择已有音频会话的应用，在 `所有进程` 中查找，或手动输入 `game.exe` 这样的名称
- 可按 `.exe` 注册整个应用，也可按 PID 单独注册当前正在运行的实例
- 应用备注、实时状态显示，以及单独暂停 / 继续
- 关闭窗口后仍在托盘中继续监控，并可从托盘执行打开 / 隐藏到托盘 / 全局暂停 / 退出
- 可设置 `启动时最小化到托盘`、`Windows 登录时自动启动`、`退出时恢复 UnfocusMute 静音的音频`
...
> **提示：** 由于代码签名证书需要成本，目前发布文件未进行 Windows 代码签名。首次运行时，Windows SmartScreen 或“未知发布者”警告可能会出现。如果你想自行确认文件完整性，请参阅[验证发布文件](#验证发布文件)部分。

## 使用前须知

UnfocusMute 基于 Windows 提供的进程名、前台窗口信息和 CoreAudio 会话来工作。
...
8. 关闭窗口后，应用会留在托盘中继续监控已注册应用。如需完全退出，请点击 `退出`。
...
如果应用没有出现在列表中，或 PID 注册的行为不符合预期，请先查看上面的[使用前须知](#使用前须知)和[查看可执行文件名](#查看可执行文件名)。

如果顶部状态变为 `需要注意`，请点击 `详情` 查看原因。音频设备、安全软件、权限策略或自动启动注册表写入限制，都可能导致部分功能受限。即使无法注册前台窗口变化通知，自动静音也会继续通过轮询工作。

如果问题仍然存在，请在 GitHub Issues 中附上 UnfocusMute 版本、Windows 版本、注册的 `.exe` 名称、是否使用 PID 注册、音频设备，以及 `详情` 中显示的消息。

---

## 安全与隐私

UnfocusMute 是完全在本地运行的应用。即使没有互联网连接也能正常工作，并且不需要管理员权限。它也不会发起自动网络请求、使用遥测、发送崩溃报告、进行远程日志记录或收集数据。

作为例外，只有当你在设置界面点击 `GitHub 仓库` 按钮时，才会在默认浏览器中打开本项目的 GitHub 仓库。

音频会话检测和静音控制只使用 Windows CoreAudio API。UnfocusMute 不会向目标进程注入代码，不会读取其内存，也不会 hook 输入。
...
如果需要直接查看或备份设置文件，请使用设置界面的 `打开配置文件夹`。文件名为 `%APPDATA%\UnfocusMute\config.json`。

你可以直接编辑它，但应用运行期间，建议尽量在应用内修改设置。保存时会先写入临时文件，再替换当前文件；无法读取的设置会先备份为 `config.invalid-<timestamp>.json`，然后恢复为默认值。
...
为保持透明，UnfocusMute 提供了一种方法，让用户可以验证上传到 GitHub Releases 的文件是否确实是 GitHub Actions 基于本仓库对应标签的源代码生成的官方构建产物。
