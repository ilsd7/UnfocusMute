...
它是 Rust 原生应用，无需额外运行时即可直接运行。当前 Windows 可执行文件约 533KB，小于 1MB。
...
如果不确定该输入什么名称，请先启动游戏，回到 Windows 后再按 `Ctrl`+`Shift`+`Esc` 打开任务管理器。将进程列表按 `CPU` 排序，通常更容易找到正在运行的游戏。右键点击该游戏并打开 `属性`，即可看到类似 `game.exe`、以 `.exe` 结尾的进程名称。
