...
- 기본 목록에서 오디오 세션이 있는 앱을 고르고, 필요하면 `전체 프로세스`로 전환하거나 `game.exe`처럼 직접 입력
...
기본 선택 목록에는 현재 오디오 세션이 있는 앱만 표시됩니다. 아직 오디오 세션이 없는 앱은 `전체 프로세스`로 전환하면 실행 중인 `.exe` 목록에서 찾을 수 있습니다.
...
3. 등록할 게임이나 앱을 실행하세요.
4. `프로세스 검색`에서 앱을 고르거나 검색어로 찾은 뒤 `등록`을 누르세요. 앱이 아직 오디오 세션을 생성하지 않았다면 `전체 프로세스`로 전환하여 실행 중인 모든 프로세스 목록을 살펴볼 수 있습니다. 목록에 없다면 `.exe` 이름을 직접 입력하세요.
