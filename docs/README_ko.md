...
- `시작 시 트레이로 최소화`, `Windows 로그인 시 자동 실행`, `종료 시 UnfocusMute가 음소거한 오디오 복원` 설정
...
따라서 드라이버, 권한 설정, 보안 프로그램이 세션 접근을 제한하는 경우에는 음소거 제어가 정상적으로 작동하지 않을 수 있습니다.
...
6. 등록된 앱을 마우스 오른쪽 버튼으로 클릭해 메모를 편집하거나 개별적으로 `일시 중지`할 수 있습니다.
...
문제가 계속되면 GitHub Issues에 제보해 주세요.

## 설정 파일

설정 파일을 직접 확인하거나 백업하려면 설정 화면의 `설정 폴더 열기` 버튼을 클릭하세요.
설정 파일이 저장되는 `%APPDATA%\UnfocusMute` 폴더가 파일 탐색기로 열립니다.

설정 파일을 직접 편집할 수는 있지만, 형식이 잘못되어 읽을 수 없는 경우 `config.invalid-<timestamp>.json`으로 백업됩니다.
앱 시작 중 문제가 발견되면 설정을 기본값으로 복구하고, 실행 중 문제가 발견되면 현재 앱 설정으로 새 설정 파일을 생성합니다.
...
릴리스 권한이 악용되거나 계정이 탈취될 경우, 공개된 코드와 다른 내용으로 빌드된 파일 또는 변조된 파일이 업로드될 수 있습니다.
