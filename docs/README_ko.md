...
UnfocusMute는 선택한 게임이나 앱이 백그라운드로 전환되면 자동으로 음소거하고, 다시 전면으로 돌아오면 음소거를 해제하는 작고 가벼운 Windows 트레이 앱입니다.

- Rust 네이티브 앱으로 빌드되어 별도 런타임 없이 바로 실행됩니다.
- 실행 파일 크기는 약 600KB입니다.
- 게임에만 한정되지 않으며, 브라우저, 메신저, 런처, 미디어 플레이어 같은 일반 앱도 등록할 수 있습니다.
- UnfocusMute가 적용한 음소거만 자동으로 해제하며, 사용자가 직접 음소거한 앱은 그대로 유지합니다.
...
최소 지원 버전은 Windows 10 버전 1703입니다.
...
4. `Alt`+`Tab`으로 다른 창에 갔다가 돌아와 보세요. 등록한 앱이 백그라운드에 있을 때는 음소거되고, 다시 전면에 오면 음소거가 해제됩니다.
...
**트레이 실행과 음소거 해제:** UnfocusMute는 등록한 앱이 다시 전면으로 돌아오거나 UnfocusMute를 종료할 때 음소거를 자동으로 해제합니다. 등록한 앱이 먼저 종료된 경우에도 음소거 상태가 남지 않도록 처리합니다. 다만 UnfocusMute가 비정상적으로 종료되면 Windows에 음소거 상태가 남을 수 있습니다. 이후 앱을 실행했는데 소리가 나지 않는다면 Windows `볼륨 믹서`에서 해당 앱의 음소거 상태를 확인하세요.

**PID 등록:** Windows가 알려 주는 오디오 세션 PID와 전면 창 PID는 서로 다를 수 있습니다. 이 때문에 PID로 등록했더라도 같은 `.exe` 이름의 창이 전면에 오면 해당 앱이 돌아온 것으로 판단해 음소거를 해제합니다. 따라서 같은 `.exe`의 창을 계속 사용하면서 특정 PID만 음소거해 두는 용도에는 적합하지 않습니다. 반대로 다른 앱에서 작업하는 동안, 같은 `.exe`의 여러 PID 중 특정 PID의 소리만 끄고 다른 PID의 소리는 유지하고 싶을 때는 유용합니다.
...
- **음소거가 해제되지 않음:** 등록한 앱으로 다시 전환해도 소리가 나지 않으면 Windows `볼륨 믹서`에서 해당 앱의 음소거 상태를 확인하세요.
