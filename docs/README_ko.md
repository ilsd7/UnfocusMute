...
- `시작 시 트레이로 최소화`, `Windows 로그인 시 자동 실행`, `종료 시 UnfocusMute가 음소거한 오디오 복원` 설정 가능
...
문제가 지속적으로 발생하면 GitHub Issues에 등록해 주세요.
...
설정 파일은 직접 편집할 수 있지만, 형식이 잘못되어 읽을 수 없는 경우 `config.invalid-<timestamp>.json`으로 백업됩니다. 이때 앱 시작 중 문제가 발견되면 설정이 기본값으로 복구되고, 실행 중 문제가 발견되면 현재 앱 설정을 기반으로 새 설정 파일이 생성됩니다.
