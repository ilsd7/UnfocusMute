...
별도 설치가 필요 없는 독립 실행형 앱이며, 별도 런타임이나 Rust, Visual Studio Build Tools, MinGW 같은 개발 도구를 설치할 필요가 없습니다.
...
3. 등록할 게임이나 앱을 실행하세요.
4. 목록에서 앱을 선택하거나 정확한 `.exe` 이름을 입력한 뒤 `등록`을 누르세요. `.exe` 이름으로 등록하면 해당 앱의 모든 오디오 세션을 함께 관리합니다. 특정 실행 인스턴스만 관리하려면 PID 등록을 사용하세요. 앱이 아직 오디오 세션을 생성하지 않았다면 `모든 프로세스`로 전환하여 실행 중인 모든 프로세스 목록을 살펴볼 수 있습니다.
