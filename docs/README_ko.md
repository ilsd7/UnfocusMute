...
- 오디오 세션 목록에서 선택하거나, 필요하면 전체 실행 프로세스 목록으로 전환하거나, `game.exe` 형식으로 직접 입력
...
UnfocusMute는 Windows가 제공하는 프로세스 이름, 전면 창 정보, CoreAudio 세션을 기준으로 동작합니다. 기본 선택 목록에는 오디오 세션이 있는 앱이 표시되며, 아직 오디오 세션을 만들지 않은 앱은 `전체 프로세스`로 전환해 실행 중인 `.exe` 목록에서 고를 수 있습니다. 다만 드라이버·권한·보안 프로그램이 세션 접근을 제한하면 음소거 제어는 여전히 제한될 수 있습니다.
...
4. `프로세스 검색` 목록을 열거나 검색어를 입력한 뒤 앱을 선택하고 `등록`을 누릅니다. 아직 오디오 세션을 만들지 않은 앱은 `전체 프로세스`로 전환해 찾고, 그래도 보이지 않으면 `.exe` 이름을 직접 입력하세요.
