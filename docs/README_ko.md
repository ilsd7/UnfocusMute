...
3. 등록할 게임이나 앱을 실행하세요.
4. 목록에서 앱을 선택하거나 정확한 `.exe` 이름을 입력한 뒤 `등록`을 누르세요. `.exe` 이름으로 등록하면 해당 앱의 모든 오디오 세션을 함께 관리합니다. 앱이 아직 오디오 세션을 생성하지 않았다면 `모든 프로세스`로 전환하여 실행 중인 모든 프로세스 목록을 살펴볼 수 있습니다.
