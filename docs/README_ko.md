...
- 등록된 앱이 백그라운드로 전환되면 오디오를 자동 음소거하고, 전면으로 돌아오면 복원
- 기본 목록에서 오디오 세션이 있는 앱을 고르고, 필요하면 `모든 프로세스`로 전환하거나 `game.exe`처럼 직접 입력
- `.exe` 단위 등록, 현재 실행 중인 인스턴스용 개별 PID 등록, `PID 보기` 지원
- 앱별 메모 작성, 실시간 음소거 상태 표시, 개별 일시 중지 / 재개
- 트레이 상주, 트레이 상태 요약, 전체 일시 중지, 설정 폴더 열기, 중복 실행 방지
- 첫 실행 시 언어를 선택하고, 이후 앱 내에서 English/한국어/日本語/简体中文/Español/Français/Português/हिन्दी/العربية 즉시 전환 가능
